...
@testset "Geodesic Interpolation" begin
    ...
    @test_throws Exception geodesic_interpolation(d_small, d_not_containing, 0.5)

    d_center1 = ValuationPolydisc([K(0)], [5])
    d_center2 = ValuationPolydisc([K(1)], [1])
    @test_throws Exception geodesic_interpolation(d_center1, d_center2, 0.5)

    d1_bounds = ValuationPolydisc([K(0)], [5])
    d2_bounds = ValuationPolydisc([K(0)], [2])
    @test_throws Exception geodesic_interpolation(d1_bounds, d2_bounds, -0.1)
    @test_throws Exception geodesic_interpolation(d1_bounds, d2_bounds, 1.1)
end
